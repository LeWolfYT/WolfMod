...
public class DiscordAssetUtils {
    ...
    public static String getRandomAsset() {
        try {
            ...
            Reference.LOG.error(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.error.config.invalid.icon.empty"));
            ex.printStackTrace();
            return "";
        }
    }
    ...
    public static DiscordAsset[] loadAssets(final String clientId, final boolean filterToMain) {
        Reference.LOG.info(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.info.discord.assets.load", clientId));
        Reference.LOG.info(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.info.discord.assets.load.credits"));
        ...
        try {
            ...
            Reference.LOG.error(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.error.discord.assets.load"));
            ex.printStackTrace();
            ...
            verifyConfigAssets();
            syncCompleted = true;
            Reference.LOG.info(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.info.discord.assets.detected", String.valueOf(ASSET_LIST.size())));
        }
    }
    ...
    private static void verifyConfigAssets() {
        ...
        for (String property : Presence.CONFIG.properties.stringPropertyNames()) {
            if ((property.equals(Presence.CONFIG.NAME_defaultIcon) || property.equals(Presence.CONFIG.NAME_defaultDimensionIcon) || property.equals(Presence.CONFIG.NAME_defaultServerIcon)) && !contains(Presence.CONFIG.properties.getProperty(property))) {
                final String newAsset = contains(Presence.CONFIG.defaultIcon) ? Presence.CONFIG.defaultIcon : getRandomAsset();
                Reference.LOG.error(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.error.config.invalid.icon.pre", Presence.CONFIG.properties.getProperty(property), property));
                Presence.CONFIG.properties.setProperty(property, newAsset);
                needsFullUpdate = true;
                Reference.LOG.error(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.error.config.invalid.icon.post", property, newAsset));
            }
        }

        if (needsFullUpdate) {
            Presence.CONFIG.save("UTF-8");
            Presence.CONFIG.read(false, "UTF-8");
        }
    }
}
