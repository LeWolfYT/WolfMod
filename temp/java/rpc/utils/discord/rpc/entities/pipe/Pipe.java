...
import com.lewolfyt.wolfmod.client.modules.rpc.utils.discord.rpc.entities.DiscordBuild;
...
import com.lewolfyt.wolfmod.client.modules.rpc.utils.discord.rpc.exceptions.NoDiscordClientException;
...
public abstract class Pipe {
    ...
    public static Pipe openPipe(IPCClient ipcClient, long clientId, HashMap<String, Callback> callbacks,
        ...
        for (int i = 0; i < 10; i++) {
            try {
                ...
                if (ipcClient.isDebugMode()) {
                    Reference.LOG.debugInfo(String.format("Searching for IPC: %s", location));
                }
                ...
                if (pipe != null) {
                    ...
                    if (ipcClient.isDebugMode()) {
                    	Reference.LOG.debugInfo(String.format("Found a valid client (%s) with packet: %s", pipe.build.name(), p.toString()));
                    	Reference.LOG.debugInfo(String.format("Found a valid user (%s) with id: %s", pipe.currentUser.getName(), pipe.currentUser.getId()));
                    }

                    // we're done if we found our first choice
                    if (pipe.build == preferredOrder[0] || DiscordBuild.ANY == preferredOrder[0]) {
                        if (ipcClient.isDebugMode()) {
                        	Reference.LOG.debugInfo(String.format("Found preferred client: %s", pipe.build.name()));
                        }
                        break;
                    }
        ...
        if (pipe == null) {
            ...
            for (int i = 1; i < preferredOrder.length; i++) {
                ...
                if (ipcClient.isDebugMode()) {
                	Reference.LOG.debugInfo(String.format("Looking for client build: %s", cb.name()));
                }
        ...
        for (int i = 0; i < open.length; i++) {
            ...
            if (open[i] != null) {
                try {
                    ...
                    // as debug info
                    if (ipcClient.isDebugMode()) {
                    	Reference.LOG.debugError(String.format("Failed to close an open IPC pipe: %s", ex));
                    }
                }
            }
        }
    ...
    public void send(Packet.OpCode op, JsonObject data, Callback callback) {
        try {
            ...
            write(p.toBytes());
            if (ipcClient.isDebugMode()) {
            	Reference.LOG.debugInfo(String.format("Sent packet: %s", p.toDecodedString()));
            }

            if (listener != null)
                listener.onPacketSent(ipcClient, p);
        } catch (IOException ex) {
        	Reference.LOG.error("Encountered an IOException while sending a packet and disconnected!");
    ...
    public Packet receive(Packet.OpCode op, byte[] data) {
        ...
        if (ipcClient.isDebugMode()) {
        	Reference.LOG.debugInfo(String.format("Received packet: %s", p.toString()));
        }
