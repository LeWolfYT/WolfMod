...
public class FileUtils {
    ...
    public static void downloadFile(final String urlString, final File file) {
        try {
            Reference.LOG.info(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.info.download.init", file.getName(), file.getAbsolutePath(), urlString));
            final URL url = new URL(urlString);
            if (file.exists()) {
                final boolean fileDeleted = file.delete();
                if (!fileDeleted) {
                	Reference.LOG.error(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.error.delete.file", file.getName()));
                }
            }

            final InputStream stream = UrlUtils.getURLStream(url);
            org.apache.commons.io.FileUtils.copyInputStreamToFile(stream, file);
            Reference.LOG.info(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.info.download.loaded", file.getName(), file.getAbsolutePath(), urlString));
        } catch (Exception ex) {
        	Reference.LOG.error(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.error.download", file.getName(), urlString, file.getAbsolutePath()));
            ex.printStackTrace();
        }
    }
    ...
    public static void loadFileAsDLL(final File file) {
        try {
        	Reference.LOG.info(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.info.dll.init", file.getName()));
            boolean isPermsSet = file.setReadable(true) && file.setWritable(true);
            if (isPermsSet) {
                System.load(file.getAbsolutePath());
            }
            Reference.LOG.info(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.info.dll.loaded", file.getName()));
        } catch (Exception ex) {
        	Reference.LOG.error(Reference.TRANSLATOR.translate("wolfmod.modules.rpc.logger.error.dll", file.getName()));
            ex.printStackTrace();
        }
    }
    ...
    public static int getModCount() {
        ...
        int modCount = Reference.IS_DEV ? 1 : 0;
        final File[] mods = new File(Reference.modsDir).listFiles();
    ...
    public static List<Class<?>> getClassNamesMatchingSuperType(final List<Class<?>> searchList, final boolean includeExtraClasses, final String... sourcePackages) {
        ...
        try {
            classList.addAll(ClassPath.from(Reference.CLASS_LOADER).getAllClasses());
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        ...
        for (String startString : sourceData) {
            ...
            try {
                ...
                if (!found && includeExtraClasses) {
                    final Class<?> extraClass = Class.forName(startString, false, Reference.CLASS_LOADER);
                    found = true;
    ...
    public static InputStream getResourceAsStream(final Class<?> fallbackClass, final String pathToSearch) {
        ...
        try {
            in = Reference.CLASS_LOADER.getResourceAsStream(pathToSearch);
        } catch (Exception ex) {
            useFallback = true;
        }
